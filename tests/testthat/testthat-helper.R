test_start_processing = function(data, x, y, NA_val, env_data, grid_extent,
...
test_calc_grid = function(x, bandwidth, cellsize, env_data, grid_extent, is_LS = FALSE){
...
test_env_vect = function(env, env_buff, env_field, grid){
...
test_spat_kde = function(x, ref, bw){
...
test_kde = function(points, ref_uq_x, ref_uq_y, bw) {
...
test_normalization = function(data, method, range = c(0, 1)){
...
test_kde_points = function(vect_x, vect_y, bw){
...
test_spat_dr = function(x, ref, bw) {
  gx = terra::crds(ref)[,1] |> unique()
  gy = terra::crds(ref)[,2] |> unique()
  # distance from each x/y ref cell center to each x/y x points (squared)
  kde_val = test_kde(x, gx, gy, bw)
  dr_val = test_kde_points(terra::crds(x)[,1], terra::crds(x)[,2], bw)
  # DR EVAL
...
test_trajectories_fun = function(data){
...
test_exposure_PO = function(data, x, y, NA_val, cellsize, group_split, env_data, env_field, env_buff,
    ...
    if (inherits(data, "data.frame")){
      if (all(!c(missing(x), missing(y)))){
        ...
        if (all(c(x_enq, y_enq) %in% colnames(data))){
          ...
          data_proj = test_start_processing(data = data, x = x_enq,
      ...
      data_proj = test_start_processing(data = data, env_data = env_data,
  ...
  if (!missing(grid_extent) && inherits(grid_extent, "SpatRaster")){
    ...
    grid_rast = test_calc_grid(x = data_proj, cellsize = cellsize,
                          env_data = env_data, grid_extent = grid_extent)
  }
  ...
  if (!missing(env_data) && inherits(env_data, "SpatVector")) {
    ...
    env_data = test_env_vect(env = env_data, env_buff = env_buff,
  ...
  for (data_i in data_iter){
    ...
    if (normalize && (!norm_group || norm_method != "range" || length(data_iter) == 1)){
      ...
      if (norm_method == "range"){
        ...
        rast_points = test_normalization(rast_points, method = norm_method)
  ...
  if (normalize && norm_method == "range" && norm_group && length(data_iter) > 1){
    ...
    act_out = test_normalization(act_out, method = "range")
  }
...
test_exposure_KDE = function(data, x, y, NA_val, cellsize, group_split, bandwidth, env_data,
  ...
  for (data_i in data_iter){
    ...
    #
    #   kde_rast = test_spat_kde(data_i, grid_crop, bandwidth)
    # } else {
    #   kde_rast =  test_spat_kde(data_i, grid_rast, bandwidth)
    # }
    ...
    if (normalize && (!norm_group || norm_method != "range" || length(data_iter) == 1)){
      ...
      kde_rast = test_normalization(kde_rast, method = norm_method)
...
test_exposure_DR = function(data, x, y, NA_val, cellsize, group_split, bandwidth, env_data,
  ...
  for (data_i in data_iter){
    ...
    dr_rast = test_spat_dr(data_i, grid_rast, bandwidth)
    ...
    if (normalize && (!norm_group || norm_method != "range" || length(data_iter) == 1)){
      ...
      # calculate normalization to 0-1 range
      dr_rast = test_normalization(dr_rast, method = norm_method)
...
test_exposure_LS = function(data, x, y, NA_val, time_data, time_unit = "mins",
  ...
  for (data_i in data_iter) {
    ...
    trajectories = terra::vect(test_trajectories_fun(data_i))
    ...
    if (!missing(time_data)){
      ...
      if(normalize && (!norm_group || norm_method != "range" || length(data_iter) == 1)) {
        ...
        duration_line_id$time_elapsed = test_normalization(duration_line_id$time_elapsed,
                                                      method = norm_method)
