#' Calculate Density Ranking exposure of point spatial data
#'
#' @description
#' `exposure_DR()` calculates activity space and environmental exposure using Density Ranking method.
#' Density Ranking utilizes Silverman's quartic kernel (see [exposure_KDE()]) and DR function from Yen-Chi Chen's repository yenchic/density_ranking:
#' \deqn{
...
exposure_DR = function(data, coords, bandwidth, cellsize, env_data, output_crs,
  ...
  #for (data_i in ifelse(n_calculations == 1, list(data_iter), list(unique(data_iter)))[[1]]){
  for (data_i in data_iter){
    ...
    # if (class(data_i) != "SpatVector"){
    #   data_i = terra::subset(data_proj, data_iter == data_i)
    # }



    dr_rast = spat_dr(data_i, grid_rast, bandwidth, n_calculations)
    ...
    if (!missing(normalize) && (!norm_group || normalize != "range" || n_calculations == 1)){
      ...
      dr_rast = normalization(dr_rast, method = normalize)

    }

    dr_rast = terra::subst(dr_rast, from = 0, to = NA)

    act_out[[index]] = dr_rast
